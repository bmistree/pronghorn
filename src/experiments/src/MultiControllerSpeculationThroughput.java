...
public class MultiControllerSpeculationThroughput
{
    ...
    public static void main (String[] args)
    {
        ...
        try {
            ...
            prong.start();
            mc_speculation_app =
                MultiControllerSpeculation.create_single_sided(ralph_globals);
            num_switches_app =
                GetNumberSwitches.create_single_sided(ralph_globals);

            prong.add_application(mc_speculation_app);
        ...
        for (HostPortPair hpp : children_to_contact_hpp)
        {
            try {
                ...
                InternalServiceFactory factory =
                        new InternalServiceFactory(MultiControllerSpeculation.factory,
                                                   ralph_globals);
                mc_speculation_app.install_remotes(factory);
            } catch(Exception e) {
                e.printStackTrace();
                assert(false);
            }
